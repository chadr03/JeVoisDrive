...
public class JeVoisInterface {
    ...
    /**
     * Returns target x position in pixels
     */
    ...
    public int parsePacket(String pkt, double rx_Time){
        ...
        final int TGT_X_TOKEN_IDX = 1;
        final int TGT_Y_TOKEN_IDX = 2;
        ...
        if(tokens.length < NUM_EXPECTED_TOKENS){
            DriverStation.reportError("Got malformed vision packet. Expected 3 tokens, but only found " + Integer.toString(tokens.length) + ". Packet Contents: " + pkt, false);
            return -1;
        }
        ...
        try {
            ...
            tgtVisible = Integer.parseInt(tokens[TGT_VISIBLE_TOKEN_IDX]);
